...
const dibujaLista = function () {
    ...
    for (const tarea of arregloTareas) {
        ...
        <tr> 
            <td>${tarea.id}</td>
            <td>${tarea.nombre}</td>
            <td><input type="checkbox" ${statusCheck} onclick="marcarTareaRealizada(${tarea.id})"><button class="btn btn-danger" onclick="eliminarTarea(${tarea.id});">X</button></td>
        </tr>`;
    }
